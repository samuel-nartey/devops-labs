...
- Your `notes-app-v1.zip` (4 files zipped: `app.js`, `package.json`, `.ebignore`, `.env.example`)
- AWS account with console access
- About 30–40 minutes
...
Create a folder called `notes-app` on your computer with these 4 files.
...
> This file is just a reference template. You do not need a local `.env` file —
> all these values are set directly in the EB console during deployment.
...
Select all 4 files individually (NOT the folder) → right-click → Compress to ZIP →
name it `notes-app-v1.zip`

The ZIP must contain the files at the root level:
```
notes-app-v1.zip
  ├── app.js
  ├── package.json
...
> Do NOT include `node_modules` — EB runs `npm install` automatically on the server.
...
> Do this first. It takes ~5 minutes to become available.
> The estimated monthly cost shown (~$15) is for 24/7 usage. For a 3-hour
> practice session you will pay approximately $0.06. Delete it when done.

1. AWS Console → search **RDS** → **Create database**
2. **Creation method:** Standard create (same as Full configuration)
3. **Engine:** PostgreSQL
4. **Template:** Dev/Test
5. **Instance configuration:**
   - Select **Burstable classes (includes t classes)**
...
10. Click **Create database** → wait ~5 minutes for status **Available**

### Add the inbound rule to the security group

After the database is created, you must open port 5432 so EB can reach it:
...
The EB wizard has 6 steps. Go through each one below.
...
| Field | Value |
|---|---|
| VPC | Select the default VPC (only one listed, e.g. `vpc-0870e3d690a7f098d`) |
| Public IP address | **Enabled** |
| Instance subnets | Tick any one subnet (e.g. `us-east-1a`) |
| Database | Leave **Enable database unchecked** — RDS was already created in Part 1 |
| Tags | Leave empty |
...
| Architecture | **x86_64** |
| Instance types | Remove `t3.small`, keep only `t3.micro` |
| AMI ID | Leave as default |

Scroll down to **Environment properties** — add all 7 by clicking
**Add environment property** for each:
...
| Field | Value |
|---|---|
| Health reporting | **Basic** (Enhanced costs extra — leave it on Basic) |
| Managed updates | Uncheck **Enable** |
| Email notifications | Leave blank |
| Deployment policy | **All at once** |
| X-Ray daemon | Leave disabled |
| S3 log storage | Leave disabled |
| Log streaming | Leave disabled |

> Environment properties also appear at the bottom of this page.
> If you missed adding them in Step 4, you can add them here instead.
...
> If you see "DB Error":
> - Confirm the RDS security group inbound rule is set (port 5432, 0.0.0.0/0)
> - Confirm all 7 environment properties are correct
...
Save the file. Select the same 4 files again, zip them into `notes-app-v2.zip`.
...
Everything else identical. Click **Next**

---

### Step 2 — Configure service access

Identical to Blue. Click **Next**

---

### Step 3 — Networking, database, and tags

Identical to Blue (Public IP enabled, one subnet, database disabled). Click **Next**

---

### Step 4 — Instance traffic and scaling

Same instance settings (Single instance, On-Demand, x86_64, t3.micro only).

Environment properties — same 7 but change two values:
...
Same as Blue (Basic health, managed updates disabled, all logging off). Click **Next**
...
Wait 3–5 minutes for Green to launch with health **OK**.
...
2. You should see a **green header** — "v2 — GREEN environment"
3. All notes you added on Blue are visible here — same RDS database
4. Each note shows "written by v1" — they were created by the Blue environment

> At this point:
> - Blue URL → v1 (blue header) — what users would see in production
> - Green URL → v2 (green header) — only you know about this URL
> - Same database — same notes visible in both environments

---

## PART 5 — The CNAME Swap

The AWS Console does not have a swap button for EB environments.
You need the EB CLI for this one command only.
...
# Link the CLI to your existing EB app (no new resources created)
eb init notes-app --platform "Node.js 18 running on 64bit Amazon Linux 2023" --region us-east-1

# THE swap command — exchanges CNAMEs between Blue and Green
...
The swap is instant. Zero downtime. No redeployment.
...
Open the Blue URL — it shows the blue header and v1 again.
Blue never stopped running — there is nothing to redeploy.
This is the core value of blue-green: rollback takes seconds, not minutes.
...
### Clean up security group (Console)
...
- RDS: Databases page should show no instances
- EC2: Instances page should show no running instances
...
Run `eb swap`. Refresh Tab 1 — it now shows:
`{"status":"ok","version":"v2","env":"green"}`

Also notice:
- The header on Tab 1 changes from blue to green
- Notes added on Blue are still there on Green (shared RDS)
- Each note has a "written by v1" or "written by v2" badge
- Rollback flips everything back in seconds
...
| Problem | Fix |
|---|---|
| "DB Error" on app load | Check RDS security group has inbound rule: PostgreSQL / 5432 / 0.0.0.0/0 |
| Still "DB Error" after SG fix | EB environment → Actions → Restart app server |
| Environment stuck in "Degraded" | EB environment → Logs → Request last 100 lines → read the error |
| Wrong env properties | EB environment → Configuration → Software → Edit → fix → Apply |
| `eb swap` says env not found | Run `eb init notes-app --region us-east-1` first |
| EC2 instance profile error in Step 2 | Create IAM role `aws-elasticbeanstalk-ec2-role` with `AWSElasticBeanstalkWebTier` policy |
| db.t3.micro greyed out in RDS | Select "Burstable classes" radio button above the dropdown first |
...
The monthly estimate AWS shows (~$15) assumes 24/7 usage. You are only paying for
the hours the resources are running. Always run Part 7 (teardown) when done.
...
3. Install the PostgreSQL client that matches the engine version you chose when creating RDS.
   Check your version on the RDS instance page under **Configuration → Engine version**, then
   use the matching command from the table below:

   | RDS engine version | Install command |
   |---|---|
   | PostgreSQL 13.x | `sudo yum install -y postgresql13` |
   | PostgreSQL 14.x | `sudo yum install -y postgresql14` |
   | PostgreSQL 15.x | `sudo yum install -y postgresql15` |
   | PostgreSQL 16.x | `sudo yum install -y postgresql16` |
   | PostgreSQL 17.x | `sudo yum install -y postgresql17` |
   | PostgreSQL 18.x | `sudo yum install -y postgresql18` |

   > ⚠️ The client version must match your engine version. Installing postgresql15 against
   > a PostgreSQL 18 server (or vice versa) can cause compatibility errors. Always check
   > your RDS engine version first and pick the matching row above.
